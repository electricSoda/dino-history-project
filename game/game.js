class Game {
    ...
    constructor(start, debugged, checkbox){
        this.started = start;
        this.debug = debugged;
        this.set_debug();
        ...
        this.pos = -100;
        this.ind = 0;
        this.info = ["1989 - Nintendo's Game Boy revolutionizes hand held gaming", "1991 - Sonic the Hedgehog is released", "1993 - Mortal Kombat starts raising questions about video games and violence in the US", "1996 - Tomb Raider is released, but many critisize the feminist portrayal of the protagonist","1997 - IBM's supercomputer Deep Blue defeats Gary Kasparov in a chess match", "2000 - The Sims is released, becomes best-selling computer game ever", "2001 - Halo is released by Microsoft, with very advanced graphics", "2002 - The U.S. army releases America's Army video game to recruit electronic gamers", "2003 - Valve releases steam, changing how players buy, download, and play games", "2006 - Nintendo Wii is a new technology to get gamers off their couches and be active", "2009 - Accessible games like Farmvill and Angry Birds make non-gamers become gamers", "2010 - Call of Duty: Black Ops becomes the best selling game"]
        this.checkbox = checkbox;
        if (this.checkbox)
            this.checkbox.changed(this.check);
    }
    ...
    update(){
        if (this.pos == -100) this.pos = width-100;
        ...
        if((this.player.isAlive() &&this.started)){
            ...
            textAlign(RIGHT);
            textSize(30);
            text(this.info[this.ind], this.pos, height/2-100);
            textAlign(LEFT);
            this.pos -= 2.5;
            if (this.pos < 0) {
                this.pos = width-100;
                this.ind = (this.ind + 1) % this.info.length;
            }
            textSize(15);
            fill(32, 33, 36);
            text("",205,585);
            ...
            textSize(32);
            fill(32, 33, 36);
            text("Press the space bar to start and restart",205,585);
        }
